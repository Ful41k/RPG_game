...
void runMission(Player* hero, int missionId) {
    int b;
    if (missionId == MISSION_SWAMP) {
        b = 1;
    } else if (missionId == MISSION_MANSION) {
        b = 2;
    } else if (missionId == MISSION_CAVE) {
        b = 3;
    } else {
        b = 4;
    }
    ...
    char* biomeName;
    if (b == 1) {
        biomeName = "Rotting Swamp";
    } else if (b == 2) {
        biomeName = "Haunted Mansion";
    } else if (b == 3) {
        biomeName = "Crystal Cave";
    } else {
        biomeName = "Dark Lord's Castle";
    }
    ...
    while (1) {
        ...
        if (c == 3) {
            ...
            if (b == 1) {
                runRottingSwamp(hero);
            } else if (b == 2) {
                runHauntedMansion(hero);
            } else if (b == 3) {
                runCrystalCave(hero);
            } else {
...
static int tryUseHealingPotion(Player* hero) {
    if (hero->potionsSmall <= 0) { 
        print_typewriter(RED "No potions!" RESET, TEXT_SPEED); 
        return 0; 
    }
    int heal = roll_d6() + 5;
    hero->potionsSmall--;
    ...
    while (hero->hp > 0) {
        ...
        if (choice == 1) {
            ...
            if (roll >= fatalStrike) {
                ...
                hero->coins += reward;
                if (reward > 0) {
                    printf(YELLOW "Loot found: %d coins.\n" RESET, reward);
                }
                *killed = 1;
                return; 
            } else {
            ...
            for (int i = 0; i < 10; i++) {
                ...
                if (rooms[i] == 1) { 
                    ...
                    if (roll == 1) {
                        ...
                        battleNormalFight(hero, "Wild Dog", 2, 1, 0, &killed);
                    } else if (roll == 3) {
                        battleNormalFight(hero, "Goblin", 3, 2, 2, &killed);
                    } else if (roll == 4) {
                        battleNormalFight(hero, "Skeleton", 4, 2, 4, &killed);
                    } else {
                        battleNormalFight(hero, "Orc", 4, 4, 6, &killed);
                    }
                }
